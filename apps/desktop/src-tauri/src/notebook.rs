...
#[derive(Default)]
pub struct NotebookHistories(Arc<Mutex<HashMap<PathBuf, NotebookHistory>>>);
...
pub async fn create_notebook(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        storage::observe_notebook(&root, histories.entry(root.clone()).or_default())
            .map(|_| ())
            .map_err(message)
    })
    .await
...
pub async fn commit_notebook(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        ...
        let mut histories = histories.lock().map_err(message)?;
        storage::commit_notebook(&root, histories.entry(root.clone()).or_default(), snapshot)
            .map_err(message)
    })
    .await
    .map_err(message)?
...
pub async fn undo_notebook(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let root = fs::canonicalize(root).map_err(message)?;
        let mut histories = histories.lock().map_err(message)?;
        storage::undo_notebook(&root, histories.entry(root.clone()).or_default()).map_err(message)
    })
    .await
    .map_err(message)?
}
...
pub async fn redo_notebook(
    ...
    tauri::async_runtime::spawn_blocking(move || {
        let root = fs::canonicalize(root).map_err(message)?;
        let mut histories = histories.lock().map_err(message)?;
        storage::redo_notebook(&root, histories.entry(root.clone()).or_default()).map_err(message)
    })
    .await
    .map_err(message)?
}
